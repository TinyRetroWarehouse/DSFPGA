function generate_bios(index, size)

   local filecontent = {}
   local input = io.open("bios"..index..".bin", "rb")
   local dwordsum = 0
   local dwordpos = 0
   while true do
   ...
   local outfile=io.open("../src/ds/ds_bios"..index..".vhd","w")
   io.output(outfile)
   ...
   io.write("\n")
   io.write("entity ds_bios"..index.." is\n")
   io.write("   port\n")
   io.write("   (\n")
   ...
   io.write("end entity;\n")
   io.write("\n")
   io.write("architecture arch of ds_bios"..index.." is\n")
   io.write("\n")
   io.write("   type t_rom is array(0 to "..(size - 1)..") of std_logic_vector(31 downto 0);\n")
...
generate_bios(7, 4096)
generate_bios(9, 1024)
