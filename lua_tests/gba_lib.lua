...
gba_clear_saves = true

function transmit_rom(filename, targetreg, transmit_dwords)
   
   -- search if savegame exists
   local path,file,extension = SplitFilename(filename)
   local savefile = gba_savegame_path..string.sub(file, 1, #file - 3).."sav"
   print(savefile)
   
   local clear_save = true
   if (gba_loadsavegame) then
      local input = io.open(savefile, "rb")
      if (input ~= nil) then
         print("Does exist")
         clear_save = false
         ...
         local flashindex = 0
         local eepromindex = 0
         local filecontent_flash = {}
         local filecontent_eeprom = {}
         while true do
            local byte = input:read(1)
            if not byte then 
               break 
            else
               if (flashindex < 131072) then
                  filecontent_flash[flashindex] = string.byte(byte)
                  flashindex = flashindex + 1
               else
                  filecontent_eeprom[eepromindex] = string.byte(byte)
         ...
         reg_set_block(filecontent_flash, ddrram.Softmap_GBA_FLASH)
         reg_set_block(filecontent_eeprom, ddrram.Softmap_GBA_EEPROM)   
      end
   end
   ...
   local namebytes = {}
   for i = 1, #savefile do
      namebytes[i] = string.byte(string.sub(savefile, i, i))
   end
   namebytes[0] = #savefile
   reg_set_block(namebytes, ddrram.Softmap_Exchange)
   ...
   if (clear_save and gba_clear_saves) then
      -- search for save string
      if (FindArray(filecontent_char, "FLASH1M_V")) then
         
         print("1 Mbit Flash")
         local saveblock = {}
         for i = 0, 131071 do
            saveblock[i] = 0xFFFFFFFF -- 0xFF should be enough
         end
         reg_set_block(saveblock, ddrram.Softmap_GBA_FLASH)
   
         reg_set(1, gameboy.Reg_GBA_flash_1m)
         
      elseif (FindArray(filecontent_char, "FLASH512_V") or FindArray(filecontent_char, "FLASH_V")) then
         
         print("512 Kbit Flash")
         local saveblock = {}
         for i = 0, 65535 do
            saveblock[i] = 0xFFFFFFFF -- 0xFF should be enough
         end
      ...
      elseif (FindArray(filecontent_char, "SRAM_V")) then
      ...
      elseif (FindArray(filecontent_char, "EEPROM_V")) then
      
         print("4 or 64 Kbit EEPROM")
         local saveblock = {}
         for i = 0, 8191 do
            saveblock[i] = 0xFFFFFFFF -- 0xFF should be enough
         end
         reg_set_block(saveblock, ddrram.Softmap_GBA_EEPROM)
   
      else
      
         print("No memory string found")
   ...
   elseif (FindArray(filecontent_char, "FLASH1M_V")) then
   ...
   --reg_set_block(filecontent, targetreg)
   
   reg_set(#filecontent_char, gameboy.Reg_GBA_MaxPakAddr)
   
   reg_set_file(filename, targetreg, 0, 0)
